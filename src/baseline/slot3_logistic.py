"""
The hyperparameters used in this Logisitc Regression Multi-label model:
...
class LogisticRegression(object):
    ...
    def __init__(self, is_training, config):
        ...
        self._input_length = tf.placeholder(tf.int32, None)
        self._targets = tf.placeholder(tf.float32, [None, config.classes])
        ...
        inputs_embedded = tf.reshape(inputs_embedded, [tf.shape(inputs_embedded)[0], -1])
        ...
        with tf.variable_scope("logistic_regression", reuse=None, initializer=initializer):
            logistic_w = tf.get_variable("_W", [config.num_steps * config.embedding_size, config.classes], dtype=data_type())
            logistic_b = tf.get_variable("_b", [config.classes], dtype=data_type())
            self._logits = tf.add(tf.matmul(inputs_embedded, logistic_w), logistic_b)
...
def main(CONST, data):
    ...
    with tf.Graph().as_default(), tf.Session(config=session_conf) as session:
        ...
        with tf.variable_scope("model", reuse=None, initializer=tf.contrib.layers.xavier_initializer()):
            training_model = LogisticRegression(is_training=True, config=config)  # model class
        ...
        if not CONST.TRAIN:
            with tf.variable_scope("model", reuse=True):  # reuse scope to evaluate model :-)
                ...
                validation_model = LogisticRegression(is_training=False, config=config)
